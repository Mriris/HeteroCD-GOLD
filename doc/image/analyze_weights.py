...
# 过滤热身轮次后的数据
filtered_orig_df = post_warmup_df.copy()

# 应用平滑处理到对数图表数据
for col in log_columns:
    # 使用高斯滤波平滑数据
    filtered_orig_df[col] = gaussian_filter1d(filtered_orig_df[col], sigma=SMOOTHING_SIGMA)
...
for col in log_columns:
    ...
    # 使用平滑后的数据绘图
    plt.plot(filtered_orig_df['epoch'], filtered_orig_df[col], label=label_map[col])
...
plt.axvspan(min_epoch, WARMUP_EPOCHS, alpha=0.2, color='gray')
plt.axvline(x=WARMUP_EPOCHS, linestyle='--', color='red', alpha=0.7)
plt.text(WARMUP_EPOCHS+1, plt.ylim()[1]*0.9, f'热身', color='red', fontsize=12)

plt.yscale('log')  # 使用对数刻度
plt.title('各损失权重原始值变化趋势（对数刻度）', fontsize=TITLE_FONTSIZE)
plt.xlabel('训练轮次', fontsize=LABEL_FONTSIZE)
plt.ylabel('权重值（对数刻度）', fontsize=LABEL_FONTSIZE)
plt.grid(True, alpha=0.3)
plt.legend(fontsize=LEGEND_FONTSIZE, loc='center left', bbox_to_anchor=(1.05, 0.5))
# 调整布局，确保图例不被裁剪
plt.tight_layout(rect=[0, 0, 0.85, 1])
...
# 保存对数图
plt.savefig('doc/image/动态权重分配/weight_log_trend.png', dpi=300, bbox_inches='tight')
