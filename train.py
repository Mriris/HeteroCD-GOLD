...
if __name__ == '__main__':
    ...
    resume_epoch = 0
    best_iou = 0
    for epoch in range(resume_epoch, opt.n_epochs):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        with open(os.path.join(opt.checkpoints_dir,opt.name,"cd_log.txt"), 'a') as f:
            f.write('Epoch: %d  best_iou: %.2f  Val loss: %.2f  score: %s\n'%(epoch, best_iou, val_loss.average(),{key: score[key] for key in score}))
